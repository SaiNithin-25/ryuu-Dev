...
def main():
    ...
    prompt = build_ryuu_dev_prompt("Write a one-line Python function that returns square of x.")
    ...
    assert out.shape[1] > input_ids.shape[1], "No new tokens were generated"
    assert len(response.strip()) > 0, "Decoded response is empty"

    print(f"[OK] Inference smoke passed")
    print(f"Checkpoint: {ckpt_path}")
    print(f"Device: {device}")
    print(f"Input tokens: {input_ids.shape[1]}")
    print(f"Output tokens: {out.shape[1]}")
    print("Sample response:")
    print(safe_text(response[:300]))
